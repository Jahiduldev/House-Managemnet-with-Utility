...
class Assign_project extends CI_Controller {
    ...
    public function index() {
        if (in_array($this->session->userdata('role_id'), array(1, 2, 3))):
            ...
            $data['active_sub_menu'] = 'Assign Project';
            ...
            $this->load->view('assign_project/assign_project', $data);
            $this->load->view('common/footer', $data);
            $this->load->view('common/js', $data);
            $this->load->view('assign_project/js_assign_project', $data);
    ...
    public function addProject() {
        if (in_array($this->session->userdata('role_id'), array(1, 2, 3))):
            ...
            $delete_result = $this->db->query("delete from assign_project where employee_id='$employee'");
            ...
            foreach ($allPost as $key => $project_id):
                ...
                $insert_result = $this->common_model->insertData('assign_project', $data_designation);
                
                $ins_result = $this->db->query("select * from assign_project_final where employee_id='$employee' and project = '$project_id'");
                $num = $ins_result->num_rows();
                
                if ($num == '0')
                $this->common_model->insertData('assign_project_final', $data_designation);   
                endforeach;
                            ...
                            <div class="col-lg-10 col-lg-offset-2">
                                ...
                                if (count($resultProject) > 0):
                                    foreach ($resultProject as $rowProject) :
                                        ...
                                        $queryProjectCheck = $this->db->query("select * from assign_project where employee_id='$emp_id' and project='$project_id'");
