...
class Home56 extends CI_Controller {
    ...
    public function index() {
        if (in_array($this->session->userdata('role_id'), array(1, 2, 3, 4))):
            redirect('dashboard');
        else:
            $data['base_url'] = $this->config->item('base_url');
            $this->load->view('login56/login', $data);
    ...
    public function login() {
        ...
        if ($login_check->num_rows()):
            ...
            $this->session->set_userdata('checklog', 'attend');
            $this->session->unset_userdata('msg_title');
            $this->session->unset_userdata('msg_body');
            if ($status == 5):
    ...
    public function logout() {
        ...
        $this->session->unset_userdata('checklog');
