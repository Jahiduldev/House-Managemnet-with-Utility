                    ...
                    <div class="panel-body">
                        <form role="form" class="cmxform form-horizontal tasi-form" id="MenuForm"  method="post"  id="commentForm" action="">
                            <div class="form-group">
                                    ...
                                    <select name="date_to" id="date_to" class="form-control" required>
										...
										<?php
											...
											for ($i = 1; $i <= 12; $i++ ) { 
												...
												if(in_array(date('Y').'-'.$i.'-'.'28', $dates)){
												
													$date_str = date("F", mktime(0, 0, 0, $i, 10));
													echo '<option value="'.date('Y').'-'.$i.'"';
													if(isset($date_to) && $date_to == date('Y').'-'.$i){														
														echo ' selected ';
													}												
                                ...
                                <div class="col-lg-6">
									<button type="submit"  onclick="submitForm('<?php echo site_url('salary_delete/salarySave'); ?>')" class="btn btn-primary pull-right col-lg-offset-1">Delete</button>
									<button type="submit"  onclick="submitForm('<?php echo site_url('salary_delete/salaryProcess'); ?>')" class="btn btn-info pull-right ">Process</button>
								</div>	
							</div>
                        </form>
                            ...
                            <table class="display table table-bordered" id="hidden-table-info">
                                ...
                                <tbody>
                                    ...
                                    foreach ($get_employee_data as $row) :
                                        ...
                                        $salary_add = 0;
                                        $query_salary_add = $this->db->query("select sum(amount) as sum_amount from employee_salary_add where emp_id='$id' and `date` like '$date_to%'");
                                        ...
                                        $salary_deduct = 0;
                                        $query_salary_deduct = $this->db->query("select sum(amount) as sum_amount from employee_salary_deduct where emp_id='$id' and `date` like '$date_to%'");
